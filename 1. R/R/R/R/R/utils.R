...
#' @param val_ratio Proportion for validation
#' @param seed Random seed
#'
#' @return List with splits
#' @export
split_data <- function(x, y, train_ratio = 0.7, val_ratio = 0.15, seed = 42) {
  ...
  list(
    ...
    validation = list(x = x[val_idx, , drop = FALSE], y = y[val_idx]),
    test = list(x = x[test_idx, , drop = FALSE], y = y[test_idx])
  )
}
...
#' @export
calc_metrics <- function(y_true, y_pred, y_prob = NULL) {
  ...
  # Confusion matrix
  cm <- table(True = y_true, Predicted = y_pred)
  ...
  names(precision) <- rownames(cm)
  names(recall) <- rownames(cm)
  names(f1) <- rownames(cm)
  ...
  if (!is.null(y_prob) && n_classes == 2) {
    if (requireNamespace("pROC", quietly = TRUE)) {
      roc_obj <- pROC::roc(y_true, y_prob[, 2])
...
print.classification_metrics <- function(x, ...) {
  ...
  cat(sprintf("Macro F1: %.2f%%\n", x$macro_f1 * 100))
  cat(sprintf("Macro Precision: %.2f%%\n", x$macro_precision * 100))
